...
# Generate Drizzle SQL migrations from schema changes
...
# Seed bookings (one-off script)
npm run seed:bookings:temp

# Normalise legacy-shaped supplier JSONB data
...
**Request flow:**
```
Routes → Middlewares (auth, rate-limit) → Controllers → Services → PostgreSQL (via Drizzle ORM)
```
...
- `src/db/schema.ts` — Drizzle table definitions and type exports (`$inferSelect` / `$inferInsert`)
- `src/db/index.ts` — Drizzle client (`db`) and `connectDB()` using `node-postgres` Pool
...
- **Schema**: `src/db/schema.ts` — 13 tables with relations and exported types
- **Migrations**: `drizzle/` directory — generated by `drizzle-kit generate`, applied by `drizzle-kit migrate`
- **Config**: `drizzle.config.ts` at project root
- **Connection**: reads `DATABASE_URL` from env (format: `postgresql://user:password@host:5432/dbname`)
- **Complex nested data** (Supplier fields, booking sub-objects) stored as JSONB columns
...
- Pinecone stores embeddings for semantic similarity search; IDs are PostgreSQL UUIDs
